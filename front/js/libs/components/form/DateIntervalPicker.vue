  ...
  <div class="input-group">
    <div class="input-group-prepend">
      <div class="input-group-text" v-if="checkboxShow">
        <input type="checkbox" value="1" :checked="value.checked" @change="onCheckedClick">
      </div>
      <button class="btn btn-outline-secondary" type="button" @click="onSubYearClick">
        <i class="fa fa-angle-double-left"></i>
      </button>
      <button class="btn btn-outline-secondary" type="button" @click="onSubMonthClick" v-if="value.type == 'month'" >
        <i class="fa fa-angle-left"></i>
      </button>
    </div>
    <input v-b-tooltip.hover :title="helpString" class="form-control text-center" type="text" :value="periodString" readonly>
    <div class="input-group-append">
      <button class="btn btn-outline-secondary" type="button" @click="onAddMonthClick" v-if="value.type == 'month'">
        <i class="fa fa-angle-right"></i>
      </button>
      <button class="btn btn-outline-secondary" type="button" @click="onAddYearClick">
        <i class="fa fa-angle-double-right"></i>
      </button>
      <button class="btn btn-outline-secondary" type="button" @click="onChangeTypeClick">
        <i class="fa" :class="{ 'fa-search-minus': (value.type == 'month'), 'fa-search-plus': (value.type == 'year') }"></i>
      </button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onCheckedClick() {
      this.sendInputNotify(true, { checkedChange: true, checked: !this.value.checked });
    }
  },
  mounted() {
    setTimeout(() => {
