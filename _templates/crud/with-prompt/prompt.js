  ...
  prompt: ({ inquirer }) => {
    ...
    if (inTestMode) {
      ...
      },
      {
        ...
        message: 'Type the field names? (separate by comma: "name, code, description, ...")',
        validate(value, state, item, index) {
          return (value.length > 0);
        }
      }];
        ...
        if (inTestMode) {
          return {
            ...
            crud_fieldObjects: [
              ...
              },
              {
                ...
                modelCamelName: `${toCamelCaseName('gl_country', 'gl_state', { upperStart: true, clearIdSuffix: true })}Model`,
                camelNameUpperNoId: toCamelCaseName('gl_country', 'gl_state', { upperStart: true, clearIdSuffix: true }),
                camelNameUpper: toCamelCaseName('gl_country', 'gl_state', { upperStart: true }),
                camelName: toCamelCaseName('gl_country', 'gl_state', { upperStart: false }),
                camelNameNoId: toCamelCaseName('gl_country', 'gl_state', { upperStart: false, clearIdSuffix: true }),
                isParentId: true,
              },
              {
                name: 'personId',
                ...
                required: false,
                modelName: 'gl_person',
                modelCamelName: `${toCamelCaseName('gl_person', 'gl_state', { upperStart: true, clearIdSuffix: true })}Model`,
                camelNameUpperNoId: toCamelCaseName('gl_person', 'gl_state', { upperStart: true, clearIdSuffix: true }),
                camelNameUpper: toCamelCaseName('gl_person', 'gl_state', { upperStart: true }),
                camelName: toCamelCaseName('gl_person', 'gl_state', { upperStart: false }),
                camelNameNoId: toCamelCaseName('gl_person', 'gl_state', { upperStart: false, clearIdSuffix: true }),
                isParentId: false,
              },
            ]
          }
        } else {
          answers.fullModelCamelNameUpper = toCamelCaseName(answers.name, answers.name, { upperStart: true, forcedUpperPreffix: true });
          answers.modelCamelNameUpper = toCamelCaseName(answers.name, answers.name, { upperStart: true });
          const questions = [];
          // inject parent field if missing
          if (answers.crud_parentName) {
            const parentFieldName = `${toCamelCaseName(answers.crud_parentName, answers.name, { upperStart: false, clearIdSuffix: true })}Id`;
            if (!answers.crud_fieldNames.find(item => item == parentFieldName)) {
          ...
          answers.crud_fieldNames.forEach((field) => {
            questions.push({
              ...
              initial: (field.endsWith('id') || field.endsWith('Id')) ? 'int' : 'string',
            });
            questions.push({
              type: 'confirm',
              name: field + '.required',
          ...
          return inquirer.prompt(questions).then(nextAnswers => {
            ...
            answers.crud_fieldNames.forEach((field) => {
              ...
              if (nextAnswers[field].modelName) {
                nextAnswers[field].modelCamelName = `${toCamelCaseName(nextAnswers[field].modelName, answers.name, { upperStart: true, clearIdSuffix: true })}Model`;
                nextAnswers[field].camelNameUpperNoId = toCamelCaseName(nextAnswers[field].modelName, answers.name, { upperStart: true, clearIdSuffix: true });
                nextAnswers[field].camelNameUpper = toCamelCaseName(nextAnswers[field].modelName, answers.name, { upperStart: true });
                nextAnswers[field].camelName = toCamelCaseName(nextAnswers[field].modelName, answers.name, { upperStart: false });
                nextAnswers[field].camelNameNoId = toCamelCaseName(nextAnswers[field].modelName, answers.name, { upperStart: false, clearIdSuffix: true });
                nextAnswers[field].isParentId = answers.crud_parentName && (answers.crud_parentName == nextAnswers[field].modelName);
                // parent
                if (nextAnswers[field].isParentId) {
                  answers.crud_parentModelName = nextAnswers[field].modelCamelName;
